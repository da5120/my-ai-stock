import streamlit as st
import yfinance as yf
import pandas as pd
import google.generativeai as genai
import urllib.request
import xml.etree.ElementTree as ET
from urllib.parse import quote

# --- 1. 設定網頁標題與外觀 ---
st.set_page_config(page_title="閒閒的鹹魚股市分析助手", layout="centered")
st.title("📈 閒閒的鹹魚股市分析助手")

# --- 2. 設定 API Key ---
# 安全寫法：教程式去 Streamlit 的 Secrets 保險箱裡面拿鑰匙
genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
model = genai.GenerativeModel('gemini-2.5-flash')

# --- 3. 建立使用者介面 (輸入框與按鈕) ---
st.write("請輸入你想查詢的台灣股票代號，AI 將為您進行技術面與新聞情緒分析。")
user_input = st.text_input("股票代號 (例如：2330, 2317, 2603)：", "2330")

if st.button("🚀 開始分析"):
    if user_input:
        stock_symbol = f"{user_input}.TW"
        
        # 顯示載入中的動畫圈圈
        with st.spinner(f"正在抓取 {stock_symbol} 的數據與最新新聞並請 AI 分析中...請稍候！"):
            try:
                # ==========================================
                # 任務 A：抓取 Google 財經新聞
                # ==========================================
                news_titles = []
                try:
                    # 幫股票代號加上 "台股" 讓搜尋更精準
                    query = quote(f"{stock_symbol} 台股")
                    url = f"https://news.google.com/rss/search?q={query}&hl=zh-TW&gl=TW&ceid=TW:zh-Hant"
                    
                    # 偽裝成一般瀏覽器去抓資料
                    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
                    response = urllib.request.urlopen(req)
                    xml_data = response.read()
                    
                    # 解析新聞資料並抓取前 5 篇
                    root = ET.fromstring(xml_data)
                    for item in root.findall('.//item')[:5]:
                        title = item.find('title').text
                        news_titles.append(f"• {title}")
                        
                    news_text = "\n".join(news_titles) if news_titles else "近期無重大新聞。"
                    
                except Exception as e:
                    news_text = "無法抓取新聞，可能遇到網路阻擋。"


                # ==========================================
                # 任務 B：抓取 Yahoo 財經歷史股價數據
                # ==========================================
                stock = yf.Ticker(stock_symbol)
                df = stock.history(period="3mo")
                
                if df.empty:
                    st.error(f"找不到 {stock_symbol} 的資料，請確認代號是否正確。")
                else:
                    # 計算技術指標
                    df['20MA'] = df['Close'].rolling(window=20).mean()
                    df['Std_dev'] = df['Close'].rolling(window=20).std()
                    df['Bollinger_Upper'] = df['20MA'] + (df['Std_dev'] * 2)
                    df['Bollinger_Lower'] = df['20MA'] - (df['Std_dev'] * 2)
                    support_level = df['Low'].min()
                    resistance_level = df['High'].max()
                    latest_data = df.iloc[-1]
                    
                    # 取出成交量變數 (轉換為千股)
                    volume_k = latest_data['Volume'] / 1000

                    # ==========================================
                    # 任務 C：在網頁畫面上顯示數據與新聞
                    # ==========================================
                    st.subheader(f"📊 {stock_symbol} 最新數據")
                    
                    # 畫出收盤價的折線圖
                    st.line_chart(df['Close'])
                    
                    # 顯示三個數據卡片
                    col1, col2, col3 = st.columns(3)
                    col1.metric("收盤價", f"{latest_data['Close']:.2f}")
                    col2.metric("20日均線", f"{latest_data['20MA']:.2f}")
                    col3.metric("成交量 (千股)", f"{volume_k:.0f}")

                    # 顯示新聞
                    st.divider()
                    st.subheader("📰 近期市場新聞")
                    st.write(news_text)

                    # ==========================================
                    # 任務 D：準備強化的 AI 提示詞並呼叫 AI
                    # ==========================================
                    analysis_prompt = f"""
                    你是一位資深的技術分析交易員。請根據以下 {stock_symbol} 的最新資料進行綜合分析：

                    【技術面數據】
                    * 收盤價：{latest_data['Close']:.2f} 
                    * 20日均線：{latest_data['20MA']:.2f} 
                    * 最新成交量：{volume_k:.0f} 千股
                    * 布林通道上軌：{latest_data['Bollinger_Upper']:.2f} / 下軌：{latest_data['Bollinger_Lower']:.2f}
                    * 近期重要支撐位：{support_level:.2f} / 壓力位：{resistance_level:.2f}

                    【近期市場新聞】
                    {news_text}

                    請綜合「技術面（含價量關係）」與「新聞情緒」，判斷現在是否為適合買進的時機。
                    報告中請務必明確指出：
                    1. 技術面目前的強弱勢，以及「價量配合」的狀況。
                    2. 新聞對目前股價可能的影響（利多/利空/中性）。
                    3. 給予投資人的具體操作建議與風險提示。
                    """
                    
                    # 呼叫 AI
                    response = model.generate_content(analysis_prompt)
                    
                    # 顯示 AI 的分析結果
                    st.divider()
                    st.subheader("🤖 AI 分析報告")
                    st.write(response.text)
                    
            except Exception as e:
                st.error(f"發生錯誤：{e}")
    else:
        st.warning("請先輸入股票代號喔！")